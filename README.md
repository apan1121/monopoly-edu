...
- 🃏 **抽卡式任務** - 3D 翻牌動畫，增加任務抽取的期待感
- 🪜 **蛇梯模式** - 經典大富翁蛇梯機制，加速或後退增加刺激感
...
2. **點擊骰子** 擲骰子
3. **玩家自動移動** 到目標格子
4. **蛇梯事件**（如果啟用蛇梯模式且停在蛇梯格）
   - 🪜 踩到梯子：顯示起點和終點，按確認後快速前進
   - 🐍 踩到蛇：顯示起點和終點，按確認後退回格子
5. **完成任務**（如果停在任務格）
   - 點擊卡片背面抽取任務
   - 卡片翻轉顯示任務內容和分數
   - 學生完成任務
...
#### 🎮 測試模式秘技
- **快捷鍵**：按 `Ctrl+K`（Mac 用 `Cmd+K`）
- **功能**：可手動選擇骰子點數 1-6，方便測試遊戲流程
- **使用時機**：僅在遊戲進行中可使用，設定模式下無效
